...
for i in range(params.studentCount + 1):
  if params.seedlabtype == "packet_sniffing":
    ...
    node.addService(pg.Execute("sh", "sudo /local/repository/setup_scripts/general.sh"))
...
if params.seedlabtype == "packet_sniffing":                 
  ...
  lan.addInterface(iface)
  node.addService(pg.Execute("sh", "sudo /local/repository/setup_scripts/general.sh"))
  node.addService(pg.Execute("sh", "sudo /local/repository/setup_scripts/packet_sniffing/target.sh"))
  rspec.addResource(node)
